...
import {
  getRemarkHugoShortcodeOptions,
  transformHugoShortcode,
  transformHugoShortcodeLinks,
} from './remarkHugoShortcode';
...
function parseCsvRow(
  ...
  while (i < content.length) {
    ...
    if (char === ',' && !isInQuote) {
      ...
      isInQuote = !isInQuote;
      if (i + 1 < content.length && !seps.includes(content[i + 1])) {
        if (i > 0 && !seps.includes(content[i - 1])) {
          currentField += char;
        }
      }
    } else {
...
function parseCellContent(
  ...
  try {
    ...
    const ast = fromMarkdown(processedContent, {
      extensions: [hugoShortcode(), gfm()],
      mdastExtensions: [hugoShortcodeFromMarkdown(options1), gfmFromMarkdown()],
    });
    ...
    if (
      ...
      (ast.children[0].type === 'mdxJsxFlowElement' ||
        ast.children[0].type === 'mdxJsxTextElement')
    ) {
    ...
    return flattenedChildren.length > 0
      ? flattenedChildren
      : [{ type: 'text', value: cellContent }];
  } catch (error) {
    // 如果解析失败，回退到纯文本
    console.warn(
      'Failed to parse cell content as Markdown:',
      cellContent,
      error,
    );
