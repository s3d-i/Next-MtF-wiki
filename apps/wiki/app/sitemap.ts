...
import { getSEOConfig, getLanguageConfigs } from '@/lib/site-config'
      ...
      try {
        ...
        for (const param of params) {
          if (param.slug && param.slug.length > 0) {
            ...
            if (path === 'docs' || path === 'converter' || path === 'about' || path === 'cup-calculator') {
              priority = 0.8
              changeFrequency = 'daily'
        ...
        console.warn(`Failed to generate sitemap for ${langConfig.code}/${subfolder}:`, error)
      }
    }
  }
